...
@section('content')
    <div class="container-fluid">
        ...
        @endif
        <form action="/class_teacher/view_filter_performance_form" method="get">
        ...
        <table id="table1" class="table">
            ...
            @foreach($students as $student)
                <tr>
                    ...
                    <td>{{$student->grade}}</td>
                    <td>
                        <form  action="{{url('/class_teacher/update_performance')}}" method="get">
                            <input type="hidden" name="id" value="{{$student->id}}">
                            <button class="btn btn-sm btn-warning" type="submit" name="submit">Update</button>
                        </form>
                    </td>
                    <td>
                        <form  action="{{url('/class_teacher/delete_performance')}}" method="post">
                            {{ method_field('DELETE') }}
                            @csrf
