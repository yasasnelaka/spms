
@extends('student_dashboard.dashboard')
@section('content')
    <br><br><br>
    <center>
    <div class="container-fluid">
        <form action="{{url('/search_student_results')}}" method="get">

                <div class="col-md-4">
                    <select class="mdb-select md-form" name="year">
                        <option value="" disabled selected>Choose year</option>
                        <option value="2018">2018</option>
                        <option value="2019">2019</option>
                        <option value="2020">2020</option>
                        <option value="2021">2021</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <select class="mdb-select md-form" name="grade">
                        <option value="" disabled selected>Choose class</option>
                        <option value="6">6</option>
                        <option value="7">7</option>
                        <option value="8">8</option>
                        <option value="9">9</option>
                        <option value="10">10</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <select class="mdb-select md-form" name="term">
                        <option value="" disabled selected>Choose Term</option>
                        <option value="1">1st term</option>
                        <option value="2">2nd term</option>
                        <option value="3">3rd term</option>
                    </select>
                </div>

            <div class="col-md-4">
                <input type="submit" class="btn btn-action" value="Search" id="submit" name="submit">
            </div>
        </form>
        <a href="/students/graph_form" class="btn btn-primary">Graphical View</a>
    </div>

    </center>
        <script>
            // Material Select Initialization
            $(document).ready(function() {
                $('.mdb-select').materialSelect();
            });
        </script>
@endsection
